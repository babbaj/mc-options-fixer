...
public class EpicOptionParser {
    private static final Map<String, Integer> NEW_TO_OLD = new HashMap<String, Integer>(){{
    ...
    public static String fix(String key) {
        ...
        Integer old = NEW_TO_OLD.get(key);
        if (old != null) {
            return String.valueOf(old);
        } else {
            throw new IllegalArgumentException("Failed to convert \"" + key + "\" to a keycode");
        }
    }
}
